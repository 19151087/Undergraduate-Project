...
namespace ESPFirebase
{
    ...
    void FirebaseApp::firebaseClientInit(void)
    {
        ...
        config.event_handler = http_event_handler;             // HTTP event handler
        config.cert_pem = FirebaseApp::https_certificate;      // SSL server certification, PEM format as string, if the client requires to verify the server's certificate server
        config.user_data = FirebaseApp::local_response_buffer; // HTTP user_data context
        config.buffer_size_tx = 4096;                          // HTTP transmit buffer size
        config.buffer_size = HTTP_RECV_BUFFER_SIZE;            // HTTP receive buffer size
